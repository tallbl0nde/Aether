...
namespace Aether::Theme {
    ...
    const Theme_T Dark = {
        ...
        [](uint32_t t){             // highlightFunc
            Colour col = Colour(0, 0, 0, 255);
            col.setG(200 + (50 * sin(ANIM_SPEED * (t/1000.0) * PI)));
            col.setB(220 + (30 * sin(ANIM_SPEED * (t/1000.0) * PI)));
            return col;
        }
    };
    ...
    const Theme_T Light = {
        ...
        [](uint32_t t){             // highlightFunc
            Colour col = Colour(0, 0, 0, 255);
            col.setG(225 + (25 * sin(ANIM_SPEED * (t/1000.0) * PI)));
            col.setB(203 + (13 * sin(ANIM_SPEED * (t/1000.0) * PI)));
            return col;
        }
    };
};
